...
void elasticRod::setOneBoundaryCondition(double position, int i, int k)
{
...
void elasticRod::readInputMesh()
{
	ifstream inFile1;
	inFile1.open("mesh_1/nodesInput.txt");
	v_nodes.clear();
	double a, b, c;
	while(inFile1 >> a >> b >> c)
	{
	...
	plateNv = v_nodes.size();
	inFile1.close();

	ifstream inFile2;
	inFile2.open("mesh_1/triangleInput.txt");
	v_triangular.clear();
	int aa, bb, cc;
	while(inFile2 >> aa >> bb >> cc)
	{
		...
		v_triangular.push_back(triangularCurrent);
	}
	plateTri = v_triangular.size();
	inFile2.close();
}
